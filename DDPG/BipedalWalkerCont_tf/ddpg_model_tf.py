    ...
    def build_network(self):
        with tf.variable_scope(self.name):
            ...
            self.q = tf.layers.dense(state_actions, units=1, 
                                    kernel_initializer=random_uniform(-f3, f3)
                                    bias_initializer=random_uniform(-f3, f3), 
                                    kernel_regularizer=tf.keras.kernel_regularizers.l2(0.01))
            
            self.loss = tf.loss.mean_squared_error(self.q_target, self.q)
...
class Agent(object):
    def __init__(self, alpha, beta, input_dims, tau, env, gamma=0.99,
        ...
        self.actor = Actor(alpha, n_actions, 'Actor', 
        ...
        self.critic = Critic(beta, n_actions, 'Critic', 
                            input_dims, self.sess, layer1_size, 
                            layer2_size, chkpt_dir=chkpt_dir)
        ...
        # Update critic and actor params as in the DDPG paper
    ...
    def remember(self, state, actions, reward, state_, done):
        self.memory.store_transition(state, actions, reward, state_, done)
    ...
    def learn(self):
        ...
        critic_value_ = self.target_critic.predict(new_state, self.target.actor.predict(new_state))
